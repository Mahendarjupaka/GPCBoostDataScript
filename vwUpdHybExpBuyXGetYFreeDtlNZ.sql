...
CREATE OR REPLACE VIEW public."vwUpdHybExpBuyXGetYFreeDtlNZ"
 ...
 SELECT DISTINCT concat('C', 'NZ', 'E', eoh_new."eventId", 'P', eoh_new.page, 'P',
    ...
    ( SELECT string_agg(t1.sku::text, ','::text ORDER BY (t1.sku::text)) AS string_agg
          ...
          WHERE t1."eventId" = eoh_new."eventId" AND t1.page = eoh_new.page AND t1."pagePosition" = eoh_new."pagePosition" AND t1."offerId" = eoh_new."offerId" AND t1."offerNo" = eoh_new."offerNumber" AND t1."isSkuActive"=true) AS "PRODUCTS",
        CASE
